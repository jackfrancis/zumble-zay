...
- relevance: how much this item needs the user's OWN attention right now. High when someone has explicitly asked for the user's participation — a review explicitly requested of them, an ASSIGNMENT to them, their own PR, or a direct mention of their username. An explicit assignment stays high even when "others_reviewing" is set: being one of several reviewers does not relieve an assignee. Relevance drops to LOW only when "waiting_on_others" is set (the user has already acted, or a review has landed and progress is on the author or a third party), so the ball is not in their court. "others_reviewing" lowers relevance only for a bystander who was neither assigned nor explicitly asked. If "review_auto_assigned" is set a bot auto-requested the review (e.g. CI assigning code owners), so treat that review-request as a weaker signal — though an actual assignment still counts as explicit.
...
- urgency: how time-sensitive it is FOR THE USER (someone is waiting on their participation, a deadline is near, or it is going stale). An explicit assignment or a human review request makes it time-sensitive, so urgency is high even when "others_reviewing" is set. If "waiting_on_others" is set, the ball is not in the user's court, so urgency is ~0. "others_reviewing" lowers urgency only for a bystander who was not assigned and had no review requested of them — then someone else is driving it. If "review_auto_assigned" is set, the user was not personally asked — a bot requested the review — so urgency stays low unless they are also an assignee.
