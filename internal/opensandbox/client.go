...
// client is a minimal HTTP client for the OpenSandbox lifecycle API — only the
// three calls this launcher needs (create, get, delete a sandbox). baseURL
// includes the version prefix (e.g. "https://opensandbox:8443/v1").
type client struct {
	baseURL string
	apiKey  string
	http    *http.Client
...
// omitted when zero (no TTL). env carries the ZZ_* injection contract.
